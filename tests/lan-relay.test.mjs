...
test("relay uploads ranged audio and synchronizes a room", async (t) => {
  ...
  follower.ws.send(JSON.stringify({ type: "command", action: "play" }));
  const promotedPreparation = await follower.next(
    (message) => message.type === "state"
      && message.state?.preparing === true
      && message.state?.leaderId === followerWelcome.clientId
      && message.state?.revision > promoted.state.revision,
  );
  follower.ws.send(JSON.stringify({
    type: "command",
    action: "ready",
    prepareId: promotedPreparation.state.prepareId,
  }));
  const resumedByNewLeader = await follower.next(
    (message) => message.type === "state"
      && message.state?.playing === true
      && message.state?.leaderId === followerWelcome.clientId
      && message.state?.revision > promoted.state.revision,
  );
  assert.equal(resumedByNewLeader.state.leaderId, followerWelcome.clientId);
  follower.ws.close();
});
...
test("moving one socket to another room fully detaches it and promotes the old room follower", async (t) => {
  ...
  const promoted = await oldRoomFollower.next(
    (message) => message.type === "state"
      && message.state?.leaderId === followerWelcome.clientId
      && message.state?.preparing === false,
  );
  assert.equal(promoted.state.playing, false);
  assert.equal(promoted.state.prepareId, "");
  assert.equal(promoted.state.readyCount, 0);
  assert.equal(promoted.state.requiredCount, 0);
  ...
  assert.equal(oldRoomAfterMove.prepareParticipants.size, 0);
  assert.equal(oldRoomAfterMove.prepareDeadline, 0);
  assert.equal(relay.rooms.get("NEW42").clients.size, 1);
  assert.equal(relay.rooms.get("NEW42").leaderId, leaderWelcome.clientId);

  oldRoomFollower.ws.send(JSON.stringify({ type: "command", action: "play" }));
  const controlledByPromotedLeader = await oldRoomFollower.next(
    (message) => message.type === "state"
      && message.state?.leaderId === followerWelcome.clientId
      && message.state?.preparing === true
      && message.state?.revision > promoted.state.revision,
  );
  assert.equal(controlledByPromotedLeader.state.requiredCount, 1);
  oldRoomFollower.ws.send(JSON.stringify({
    type: "command",
    action: "ready",
    prepareId: controlledByPromotedLeader.state.prepareId,
    ready: true,
  }));
  const resumedByPromotedLeader = await oldRoomFollower.next(
    (message) => message.type === "state"
      && message.state?.leaderId === followerWelcome.clientId
      && message.state?.playing === true
      && message.state?.prepareId === controlledByPromotedLeader.state.prepareId,
  );
