...
export function MineradioPlayer() {
  ...
  const roomPlaybackUnlockSequenceRef = useRef(0);
  ...
  const primeRoomPlayback = useCallback((fromUserGesture = false) => {
    ...
    if (roomPlaybackUnlockedElementRef.current === audio || !audio.paused) {
      roomPlaybackUnlockedElementRef.current = audio;
      setNeedsUnlock(false);
      return Promise.resolve(true);
    }
    if (roomPlaybackUnlockPromiseRef.current) return roomPlaybackUnlockPromiseRef.current;
    // Muted autoplay may be allowed even though the later audible scheduled
    // start is still blocked. Require a real device gesture before declaring
    // this media element unlocked.
    if (!fromUserGesture) {
      setNeedsUnlock(true);
      setNotice("此设备需要先点击“启用声音”，完成同步播放授权。");
      return Promise.resolve(false);
    }

    const sequence = ++roomPlaybackUnlockSequenceRef.current;
    const wasMuted = audio.muted;
    suppressRoomMediaEventsRef.current = true;
    audio.muted = true;

    let playAttempt: Promise<void>;
    try {
      playAttempt = Promise.resolve(audio.play());
    } catch {
      playAttempt = Promise.reject(new Error("room_media_unlock_failed"));
    }

    const operation = playAttempt.then(
      () => {
      ...
      () => {
        setNeedsUnlock(true);
        setNotice(fromUserGesture
          ? "浏览器仍阻止播放，请检查静音开关或媒体权限。"
          : "此设备需要先点击“启用声音”，完成同步播放授权。");
        return false;
      },
    ).then((unlocked) => {
      audio.muted = wasMuted;
      if (roomPlaybackUnlockPromiseRef.current === operation) {
        roomPlaybackUnlockPromiseRef.current = null;
      }
      window.setTimeout(() => {
        if (roomPlaybackUnlockSequenceRef.current === sequence) {
          suppressRoomMediaEventsRef.current = false;
        }
      }, 0);
      return unlocked;
    });
    roomPlaybackUnlockPromiseRef.current = operation;
    return operation;
  }, []);
  ...
  useEffect(
    () => () => {
      ...
      prepareControllerRef.current?.abort();
      audioErrorControllerRef.current?.abort();
      roomPlaybackUnlockSequenceRef.current += 1;
      roomPlaybackUnlockPromiseRef.current = null;
      roomPlaybackUnlockedElementRef.current = null;
  ...
  const togglePlayback = useCallback(async () => {
    ...
    try {
      if (mode === "room") {
        if (room.state?.playing || room.state?.preparing) {
          room.sendCommand({
            action: "progress",
            position: Math.max(0, audio.currentTime || 0),
            advancing: !audio.paused && !audio.ended,
          });
          room.sendCommand({ action: "pause" });
          return;
        }
        // Invoke both unlock attempts inside the click task. In particular, iOS
        // WebKit will reject a delayed media.play() even when the room later
        // reaches its synchronized scheduledAt timestamp.
        const playbackGate = primeRoomPlayback(true);
        void ensureAudioGraph().catch(() => false);
        if (!await playbackGate) return;
        room.sendCommand({
          action: "progress",
          position: Math.max(0, audio.currentTime || 0),
          advancing: !audio.paused && !audio.ended,
        });
        room.sendCommand({ action: "play" });
        return;
      }
  ...
  const unlockAudio = useCallback(async () => {
    ...
    try {
      const playbackGate = primeRoomPlayback(true);
      const graphGate = ensureAudioGraph();
      if (!await playbackGate) return;
      await graphGate;
  ...
  }, [ensureAudioGraph, mode, primeRoomPlayback, room]);
