(function installMineradioClassicWebBridge() {
  ...
  function reconcileRoomPlayback(state, forceSeek) {
    ...
    if (state.playing) {
      var scheduledPrepareId = String(state.prepareId || "");
      ...
      if (startDelay > 20) {
        ...
        scheduledPlayTimer = window.setTimeout(function () {
          scheduledPlayTimer = 0;
          playForRoomSync(media, scheduledPrepareId);
        }, startDelay);
      } else {
        window.clearTimeout(scheduledPlayTimer);
        scheduledPlayTimer = 0;
  ...
  function attachAudioEvents() {
    ...
    media.addEventListener("seeked", function () {
      ...
      if (!applyingRoomState) sendCommand("seek", media.currentTime);
    });
    media.addEventListener("timeupdate", function () {
      if (applyingRoomState || Date.now() - lastPositionSentAt < 1000) return;
  ...
  function canResumeFollowerFromGesture() {
    if (!joinedRoom || bridge.sync.leader || applyingRoomState || !lastRoomState) return false;
    if (!lastRoomState.playing || !lastRoomState.track || !window.audio || !window.audio.paused) return false;
    return currentDescriptorId() === String(lastRoomState.track.id || "");
  }
    ...
    ["togglePlay", "nextTrack", "prevTrack", "shuffleQueue", "clearQueue"].forEach(function (name) {
      ...
      window[name] = function classicRoomGuardedControl() {
        ...
        if (name === "togglePlay" && joinedRoom && bridge.sync.leader && window.audio && window.audio.src) {
          ...
          if (lastRoomState && (lastRoomState.playing || lastRoomState.preparing)) sendCommand("pause");
