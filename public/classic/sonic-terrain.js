 ...
 * Modified for the MR//ROOM browser/LAN build on 2026-08-01.
 * Released under GPL-3.0-or-later; see the repository LICENSE file.
 * Only the visual layer is embedded here; player, login and server logic stay Mineradio-native.
 */
(function (global) {
  'use strict';

  var previous = global.MineradioSonicTerrain || global.MineradioSonicTopography;
  ...
  function sonicPaletteHex(value, fallback) {
    if (Array.isArray(value) && value.length >= 3) {
      return '#' + value.slice(0, 3).map(function (part) {
        return Math.round(clamp(Number(part) || 0, 0, 255)).toString(16).padStart(2, '0');
      }).join('');
    }
    if (value && typeof value === 'object' && value.r != null && value.g != null && value.b != null) {
      var scale = Number(value.r) <= 1 && Number(value.g) <= 1 && Number(value.b) <= 1 ? 255 : 1;
      return '#' + [value.r, value.g, value.b].map(function (part) {
        return Math.round(clamp((Number(part) || 0) * scale, 0, 255)).toString(16).padStart(2, '0');
      }).join('');
    }
  ...
  function sonicUsesCustomGroundColors(fx) {
    ...
    return theme !== 'cover' || !!(fx && fx.sonicGroundColorMode === 'custom');
  }

  function sonicCoverGroundTheme(fx) {
    var stage = global.stageLyrics || {};
    var palette = state.framePalette || stage.coverPalette || stage.palette || {};
  ...
  function sonicCustomGroundTheme(fx) {
    ...
    var theme = String((fx && fx.sonicTerrainTheme) || 'cover').toLowerCase();
    var themed = {
      aurora: { base: '#040b12', cool: '#57ffb4', warm: '#b268ff', accent: '#5ae7d0' },
      ocean: { base: '#031122', cool: '#2daaff', warm: '#6b71ff', accent: '#43eeeb' },
      mono: { base: '#090a0d', cool: '#eef1f7', warm: '#9198a4', accent: '#ffffff' }
    }[theme];
    var base1 = new THREE.Color(themed ? themed.base : sonicHex(fx, 'sonicGroundBaseColor', DEFAULT_GROUND_BASE_COLOR));
    return {
      base1: base1,
      base2: base1.clone().lerp(new THREE.Color('#ffffff'), 0.12),
      coolCore: new THREE.Color(themed ? themed.cool : sonicHex(fx, 'sonicGroundCoolColor', DEFAULT_GROUND_COOL_COLOR)).lerp(tint, fx && fx.visualTintMode === 'custom' ? 0.08 : 0.0),
      warmCore: new THREE.Color(themed ? themed.warm : sonicHex(fx, 'sonicGroundWarmColor', DEFAULT_GROUND_WARM_COLOR)).lerp(tint, fx && fx.visualTintMode === 'custom' ? 0.05 : 0.0),
      ripple: new THREE.Color(themed ? themed.accent : sonicHex(fx, 'sonicGroundAccentColor', DEFAULT_GROUND_ACCENT_COLOR))
    };
  }
  ...
  function terrainIntensity(fx) {
    return clamp(Number(fx && fx.sonicTerrainIntensity) || 1, 0.4, 2.5);
  }

  function terrainResponse(fx) {
    return clamp(Number(fx && fx.sonicTerrainResponse) || 1, 0.35, 2);
  }
  ...
  function smoothGroundAudio(target, fx, dt) {
    var motionFallback = DEFAULT_GROUND_MOTION_SPEED * terrainResponse(fx);
    var motionSpeed = sonicNumber(fx, 'sonicGroundMotionSpeed', motionFallback, 0, 100);
  ...
  function buildTerrainVertexShader() {
    return [
      ...
      '  float idleShape=mix(baseNoise,wave,uSmoothness*0.5+0.2);',
      '  float idlePeaks=pow(smoothstep(0.52,0.92,baseNoise),2.0)*(1.2+rnd*1.8);',
      '  float idleElevation=(0.48+idleShape*2.10+idlePeaks)*globalFalloff;',
  ...
  function buildTerrainFragmentShader() {
    return [
      ...
      '  float normElevation=clamp(vElevation/7.0,0.0,1.0);',
      ...
      '    finalColor=mix(cBase2,currentGlow,max(0.22,topIntensity));',
  ...
  function buildTerrainMesh(settings) {
    ...
    for (var x = 0; x < settings.gridSize; x++) {
      for (var z = 0; z < settings.gridSize; z++) {
        var matrixOffset = n++ * 16;
        matrices[matrixOffset] = 1;
        matrices[matrixOffset + 5] = 1;
        matrices[matrixOffset + 10] = 1;
        matrices[matrixOffset + 12] = x * settings.spacing - offset;
        matrices[matrixOffset + 13] = 0.5;
        matrices[matrixOffset + 14] = z * settings.spacing - offset;
        matrices[matrixOffset + 15] = 1;
      }
    }
    if (typeof mesh.instanceMatrix.setUsage === 'function' && THREE.StaticDrawUsage != null) {
  ...
  function applyLayout(fx) {
    ...
    state.root.scale.set(layout.scale, layout.scale * (1.55 + terrainIntensity(fx) * 0.42), layout.scale);
  }
  ...
  function ensureLayer(scene, fx) {
    if (!scene || typeof scene.add !== 'function' || typeof THREE === 'undefined') return;
  ...
  function syncTheme(mat, fx, audio, dt) {
    ...
    var glowFallback = DEFAULT_GROUND_GLOW * clamp(terrainIntensity(fx), 0.6, 1.45);
    var glow = sonicNumber(fx, 'sonicGroundGlow', glowFallback, 0, 100);
  ...
  function syncTerrainUniforms(fx, audio, dt, time) {
    ...
    var amplitudeFallback = DEFAULT_GROUND_AMPLITUDE * terrainIntensity(fx);
    var amplitude = sonicNumber(fx, 'sonicGroundAmplitude', amplitudeFallback, 0, 100);
  ...
  function updateFloatingBlocks(fx, audio, dt, time) {
    ...
    var floatingFallback = DEFAULT_FLOATING_BLOCK_INTENSITY * terrainIntensity(fx);
    var intensity = sonicNumber(fx, 'sonicGroundFloatingIntensity', floatingFallback, 0, 100) / 100;
  ...
  function update(dt, ctx) {
    ...
    var fx = ctx.fx || {};
    var scene = ctx.scene;
    state.framePalette = ctx.palette || ctx.coverPalette || null;
    ...
    state.opacity += (1 - state.opacity) * Math.min(1, dt * 3.0);
    updateSonicRotation(fx, dt, ctx);
    applyLayout(fx);
    state.root.visible = true;
    var motionFallback = DEFAULT_GROUND_MOTION_SPEED * terrainResponse(fx);
    state.sonicTime += dt * (0.45 + sonicNumber(fx, 'sonicGroundMotionSpeed', motionFallback, 0, 100) * 0.017);
  ...
  function onPresetChange(prev, next, ctx) {
    if (prev === INDEX && next !== INDEX && state.root) {
      state.opacity = 0;
      state.root.visible = false;
    }
    if (next === INDEX && state.root) {
      applyLayout((ctx && ctx.fx) || {});
    }
    return next === INDEX;
  }
  ...
  global.MineradioSonicTerrain = api;
  global.MineradioSonicTopography = api;
