...
if (-not $jdkHome) {
  throw 'Missing portable JDK. Expected tools\jdk21\jdk-*'
}
if (-not (Test-Path $sdkRoot)) {
  throw 'Missing Android SDK. Expected tools\android-sdk'
}
if (-not (Test-Path $gradleBat)) {
  throw 'Missing Gradle. Expected tools\gradle\gradle-8.10.2\bin\gradle.bat'
}
...
& $gradleBat -p (Join-Path $root 'android') assembleDebug

$sourceApk = Join-Path $root 'android\app\build\outputs\apk\debug\app-debug.apk'
$outputDir = Join-Path $root 'outputs'
New-Item -ItemType Directory -Force -Path $outputDir | Out-Null
Copy-Item -Force $sourceApk (Join-Path $outputDir 'Mineradio-Android16-debug.apk')
Write-Output "APK: $outputDir\Mineradio-Android16-debug.apk"
